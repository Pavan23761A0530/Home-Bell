...
exports.addServiceOffering = async (req, res) => {
    try {
        ...
        const serviceDoc = await Service.findById(serviceId).select('basePrice pricingType');
        if (!serviceDoc) {
            return res.status(404).json({ success: false, error: 'Service not found' });
        }

        const adminSetPrice = Number(serviceDoc.basePrice) || 0;
        const adminPricingType = serviceDoc.pricingType || 'fixed';

        const offering = await ProviderService.create({
            provider: profile._id,
            service: serviceId,
            providerPrice: adminSetPrice,
            pricingType: adminPricingType,
            isActive: true
        });
...
exports.updateServiceOffering = async (req, res) => {
    try {
        ...
        const serviceDoc = await Service.findById(req.params.serviceId).select('basePrice pricingType');
        if (!serviceDoc) {
            return res.status(404).json({ success: false, error: 'Service not found' });
        }

        // Always enforce the admin-set price and pricing type
        offering.providerPrice = Number(serviceDoc.basePrice) || 0;
        offering.pricingType = serviceDoc.pricingType || 'fixed';
