...
const ProviderServices = () => {
    ...
    const handleSaveService = async () => {
        if (!selectedServiceId || !customPrice || !pricingType) return;
        ...
        try {
            ...
            if (editingService) {
                ...
                await api.put(`/providers/services/${selectedServiceId}`, {
                    fixedRate: pricingType === 'fixed' ? priceNum : undefined,
                    hourlyRate: pricingType === 'hourly' ? priceNum : undefined,
        ...
        <div className="space-y-6">
            ...
            >
                <div className="space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">{pricingType === 'fixed' ? 'Fixed Price (₹)' : 'Hourly Rate (₹)'}</label>
                        <input
                            type="number"
                            value={customPrice}
                            onChange={(e) => setCustomPrice(e.target.value)}
                            className="w-full p-2 border border-gray-300 rounded-lg"
                            placeholder={pricingType === 'fixed' ? "e.g. 500" : "e.g. 100"}
                        />
                    </div>
                    {selectedService && (
