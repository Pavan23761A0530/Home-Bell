...
const ProviderDashboard = () => {
        ...
        try {
            ...
            if (jobsRes.data.success) {
                const jobs = (jobsRes.data.data || []).map(j => ({
                    ...j,
                    status: j.statusNormalized || j.status
                }));
                console.log('Provider bookings status values:', jobs.map(j => j.status));
                const visibleJobs = jobs.filter(j => ['pending', 'ongoing', 'assigned'].includes(j.status));
    ...
    const handleJobAction = async (jobId, status) => {
        try {
            await api.put(`/bookings/${jobId}/status`, { status });
            // Optimistically update UI
            setActiveJobs(prev => prev.map(job => job._id === jobId ? { ...job, status } : job));
            if (status === 'completed') {
    ...
    const confirmAssign = async () => {
        ...
        try {
            ...
            if (!res.data?.success) {
                throw new Error(res.data?.error || 'Assignment failed');
            }
            ...
            if (jobsRes.data.success) {
                ...
                console.log('After assignment, bookings status values:', jobs.map(j => j.status));
                const visibleJobs = jobs.filter(j => ['pending', 'ongoing', 'assigned'].includes(j.status));
                setActiveJobs(visibleJobs);
            }
                                ...
                                <div className="flex flex-col md:flex-row gap-6">
                                    <div className="flex-grow space-y-3">
                                        ...
                                        <div className="text-right">
                                                ...
                                                <Badge
                                                variant={job.status === 'ongoing' || job.status === 'in-progress' ? 'warning' : 'primary'}
                                                    className="mt-1 capitalize"
                                                >
                                                {job.status ? job.status.replace('-', ' ') : 'Status'}
                                                </Badge>
                                            </div>
                                        </div>
                                    ...
                                    <div className="flex flex-row md:flex-col justify-end gap-2 md:w-48 border-t md:border-t-0 md:border-l border-neutral-100 pt-4 md:pt-0 md:pl-6">
                                        {(['ongoing', 'in-progress'].includes(job.status)) && (
                                            <Button
